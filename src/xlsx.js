...
export async function writeObjectsXlsx(filePath, rows, { sheetName = 'Companies', headers } = {}) {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet(sheetName);
  const finalHeaders = headers ?? collectHeaders(rows);

  if (finalHeaders.length > 0) {
    worksheet.columns = finalHeaders.map((header) => ({ header, width: 12 }));
  }

  for (const row of rows) {
    worksheet.addRow(finalHeaders.map((header) => normalizeCellValue(row?.[header])));
  }
