...
export function formatScbDate(targetDate) {
  if (/^\d{8}$/.test(targetDate)) return targetDate;
  const isoMatch = /^(\d{4})-(\d{2})-(\d{2})$/.exec(targetDate);
  if (!isoMatch) throw new Error('SCB date must use YYYY-MM-DD or YYYYMMDD format.');
...
export function formatOutputDate(targetDate) {
  ...
  if (!compactMatch) throw new Error('Output date must use YYYY-MM-DD or YYYYMMDD format.');
...
function buildExactDateQuery(variableName, targetDate, options = {}) {
  ...
  const query = {
    Variabler: [
      { Variabel: variableName, Operator: 'FranOchMed', Varde1: scbDate, Varde2: '' },
      { Variabel: variableName, Operator: 'TillOchMed', Varde1: scbDate, Varde2: '' },
    ],
  };
...
async function resolveScbCredentials({ envPath = '.env', scbDir = 'scb' } = {}) {
  ...
  if (configuredPfxPath) {
    ...
    const pfxEntry = entries.find((entry) => entry.isFile() && entry.name.toLowerCase().endsWith('.pfx'));
    if (!pfxEntry) throw new Error(`Could not find a .pfx file in ${scbDir}.`);
    pfxPath = path.resolve(scbDir, pfxEntry.name);
  }

  return { passphrase, pfxPath };
}
    ...
    const request = https.request(
      ...
      (response) => {
        ...
        response.on('end', () => {
          ...
          reject(new Error(`SCB request to ${pathname} failed with status ${response.statusCode ?? 'unknown'}: ${responseBody}`));
        });
      },
    );
...
async function countCompaniesByQuery(query, options = {}) {
  ...
  const responseBody = await scbRequestWithRetry('/nv0101/v1/sokpavar/api/je/raknaforetag', query, { pfxBuffer, passphrase });
  const count = Number.parseInt(responseBody.trim(), 10);
  if (!Number.isFinite(count)) throw new Error(`SCB count response was not a number: ${responseBody}`);
  return count;
}
...
async function fetchCompaniesPageByQuery(query, options = {}) {
  const { passphrase, pfxPath } = await resolveScbCredentials(options);
  const pfxBuffer = await readFile(pfxPath);
  const responseBody = await scbRequestWithRetry('/nv0101/v1/sokpavar/api/je/hamtaforetag', query, { pfxBuffer, passphrase });
...
async function fetchCompaniesByQuery(query, targetDate, options = {}) {
  ...
  if (count > SCB_MAX_ROWS_PER_REQUEST) {
    throw new Error(`SCB returned ${count} rows for ${formatOutputDate(targetDate)}. That exceeds the ${SCB_MAX_ROWS_PER_REQUEST} row limit per request.`);
  }
  if (count === 0) return [];
...
async function fetchCompaniesByRegistrationDateSegment(targetDate, options = {}, prefix = '') {
  const query = buildExactRegistrationDateQuery(targetDate, { orgNumberPrefix: prefix });
  const count = await countCompaniesByQuery(query, options);
  if (count === 0) return [];
  if (count <= SCB_MAX_ROWS_PER_REQUEST) return fetchCompaniesPageByQuery(query, options);
  if (prefix.length >= 10) throw new Error(`Could not segment ${formatOutputDate(targetDate)} any further for prefix ${prefix}.`);
...
export async function saveCompaniesXlsx(companies, targetDate, { outputDir = 'raw' } = {}) {
  ...
  const absoluteOutputDir = path.resolve(outputDir);
  const filePath = path.join(absoluteOutputDir, datedFileName);
  await writeObjectsXlsx(filePath, companies, { sheetName: 'Companies' });
  return filePath;
}
